...
def gen_Fattree(args):
    ...
    gpu_num = edge_switch_num * k_half
    servers = (int) (gpu_num / args.gpu_per_server)
    nv_switch_num = servers * args.nv_switch_per_server

    switch_nodes = (int) (core_switch_num + aggr_switch_num + edge_switch_num + nv_switch_num)
    nodes = (int) (gpu_num + switch_nodes)
    links = (int) (aggr_switch_num * k + edge_switch_num * k_half + servers * args.nv_switch_per_server * args.gpu_per_server)

    file_name = "Fattree_"+str(k)+"_"+args.bandwidth+"_"+args.gpu_type
    with open(file_name, 'w') as f:
        print(file_name)
        first_line = str(nodes)+" "+str(args.gpu_per_server)+" "+str(nv_switch_num)+" "+str(switch_nodes-nv_switch_num)+" "+str(links)+" "+str(args.gpu_type)
        f.write(first_line)
        f.write('\n')
        ...
        curr_node_id = gpu_num
        nv_switch = list(range(curr_node_id, curr_node_id + nv_switch_num))
        curr_node_id += nv_switch_num
        ...
        for i in range(gpu_num):
            cnt_gpu = cnt_gpu + 1
            if cnt_gpu > args.gpu_per_server:
                cnt_gpu = 1
